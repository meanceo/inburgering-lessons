# Inburgering A2 HTML Lessons - Version 4
...
The interface follows the v1/v2 design pattern:
...
Version 4 is an existing package. Future automation runs must not overwrite this folder or append new lesson material into it.
...
No-reuse rule: do not reuse speaking images from older versions or earlier runs. Create fresh version-specific image assets for the new package, store them in the matching new version folder, and use filenames that start with the new version, for example `vNNN-speaking-...`. Before finishing, run the checks with the new version paths:
...
If either checker fails, fix the Markdown or media references before considering the package complete.
